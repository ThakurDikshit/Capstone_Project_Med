...
def runXGB(train_X, train_y, test_X, test_y=None, feature_names=None, extra_X=None, seed_val=0, num_rounds=200):
	...
	if test_y is not None:
		...
		model = xgboost.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=300)
	else:
	...
	if feature_names is not None:
		...
		model.dump_model('xgbmodel.txt', 'xgboost.fmap', with_stats=True)
		importance = model.get_fscore(fmap='xgboost.fmap')
	...
	if extra_X is not None:
		xgtest = xgboost.DMatrix(extra_X)
...
if __name__ == "__main__":
	...
	train = pd.read_csv("train_with_outcome.csv") 
	...
	patient = pd.read_csv("Patient_Profile.csv", na_values=['None',''])
	...
	first_camp_details = pd.read_csv("First_Health_Camp_Attended.csv")
	...
	third_camp_details = pd.read_csv("Third_Health_Camp_Attended.csv")
	...
	out_df.to_csv("sub_thakur.csv", index=False)
